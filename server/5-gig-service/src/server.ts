...
import { IAuthPayload, winstonLogger } from '@ahadg/jobber-shared';
...
const usersErrorHandler = (app: Application): void => {
  app.use((error: IErrorResponse, _req: Request, res: Response, next: NextFunction) => {
    log.log('error', `GigsService ${error.comingFrom}:`, error);
...
const startServer = (app: Application): void => {
  try {
    ...
    httpServer.listen(SERVER_PORT, () => {
      log.info(`Gigs server running on port ${SERVER_PORT}`);
    });
  } catch (error) {
    log.log('error', 'GigssService startServer() method error:', error);
  }
};
