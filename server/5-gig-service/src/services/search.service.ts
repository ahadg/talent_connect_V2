...
import { IHitsTotal, IPaginateProps, IQueryList, ISearchResult } from '@ahadg/jobber-shared';
...
const gigsSearch = async (
  ...
  const result: SearchResponse = await elasticSearchClient.search({
    ...
    // recomendation from elastic search, by default it will look and return only 10k docs
    // we have to explicity describe to return certain docs
